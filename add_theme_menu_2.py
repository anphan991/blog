...
content = re.sub(
    r'<a href="https://www\.youtube\.com/watch\?v=dQw4w9WgXcQ"[^>]*className="([^"]*)".*?<Power[^>]*/>.*?<span[^>]*>.*?DISCONNECT<br/>\.SYS.*?</span>.*?</a>',
    r'<button onClick={() => setShowThemeMenu(true)} className="\1 border-none bg-transparent outline-none"><Power size={52} strokeWidth={1} className="text-red-500/80 group-hover:text-red-400 transition-colors drop-shadow-[0_0_10px_rgba(239,68,68,0.6)] animate-pulse" /><span className="text-[10px] md:text-sm font-[\'Share_Tech_Mono\',_monospace] bg-black/50 text-red-500 px-2 py-1 group-hover:bg-red-500 group-hover:text-black text-center leading-tight mt-1">DISCONNECT<br/>.SYS</span></button>',
    content,
    flags=re.DOTALL
)
        ...
        {showThemeMenu && (
          ...
          >
            <button 
              ...
              className="absolute top-8 right-12 text-cyan-500 font-['Share_Tech_Mono',_monospace] text-sm md:text-base hover:text-white transition-colors uppercase tracking-widest bg-transparent border-none cursor-pointer"
            >
              [ ABORT_SEQUENCE ]
            </button>
