...
with open('app/globals.css', 'r', encoding='utf-8') as f:
    css = f.read()
css = re.sub(r'\.khz-cursor\s*\{[^\}]*\}', '.khz-cursor { position: fixed; top: 0; left: 0; width: 6px; height: 6px; background-color: var(--khz-primary, #A9A9A2); border-radius: 50%; pointer-events: none; z-index: 999999 !important; transform: translate(-50%, -50%); }', css, 1)
css = re.sub(r'\.khz-cursor-ring\s*\{[^\}]*\}', '.khz-cursor-ring { position: fixed; top: 0; left: 0; width: 36px; height: 36px; border: 1px solid var(--khz-primary, #A9A9A2); border-radius: 50%; pointer-events: none; z-index: 999998 !important; transform: translate(-50%, -50%); transition: width 0.2s, height 0.2s; }', css, 1)
...
old_footer_left = r'<div>\s*<span className="font-serif text-xl text-\[#F5F5F0\] block">\|\|\|\|\|\|\|\|\|\|</span>.*?</div>'
...
page = re.sub(old_footer_left, new_footer_left, page, flags=re.DOTALL)
...
print("Done all modifications successfully")
