...
export default function Home() {
  // --- STATE handle Easter Eggs (Tối ưu nhất: Dùng state) ---
  const [clickCount, setClickCount] = useState(0); // Đếm click Status -> BSOD
  const [bsodState, setBsodState] = useState(false); // Trạng thái BSOD
  const hackerControls = useAnimation(); // Điều khiển Hacker Mode
  const holdTimeout = useRef<NodeJS.Timeout | null>(null); // Xử lý nhấn giữ chuột

  // --- 1. Hiệu ứng spam click Status Badge (Tối ưu: Chỉ logic & CSS) ---
  const handleStatusClick = () => {
    if (bsodState) {
      setBsodState(false); // Reset nếu đang BSOD
      setClickCount(0);
      return;
    }
    ...
    // Nếu bấm quá nhanh (tốc độ spam), kích hoạt BSOD
    if (newCount > 5) {
      setBsodState(true);
    }
    
    // Reset đếm sau 2 giây nếu không bấm tiếp
    setTimeout(() => setClickCount(0), 2000);
  };

  // --- 2. Hiệu ứng nhấn giữ tên 'An Phan' (Tối ưu: Chỉ thay đổi class CSS) ---
  const startHackerHold = () => {
    holdTimeout.current = setTimeout(() => {
      // Kích hoạt Hacker Mode: Chuyển màu & font
      document.documentElement.classList.add('dark', 'hacker-mode');
      hackerControls.start({
        opacity: [0, 1],
        transition: { duration: 0.2 },
      });
    }, 1000); // Nhấn giữ 1 giây
  };
...
return (
    // THAY ĐỔI: Bỏ justify-center và items-center, thêm py-20 để có khoảng trống trên dưới
    <main className="relative min-h-screen bg-[#0a0a0a] text-slate-200 overflow-y-auto z-0 font-sans p-4 md:p-8 flex flex-col items-center pt-20 md:pt-32 transition-colors duration-500">
      ...
      {/* Background Gradient - Giữ nguyên FIXED để khi scroll các đốm sáng vẫn đứng yên, tạo hiệu ứng Parallax cực sang */}
      ...
      {/* BENTO BOX GRID CONTAINER - mx-auto để căn giữa theo chiều ngang */}
      <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-3 gap-4 auto-rows-[minmax(180px,auto)] mb-20">
        
        {/* BENTO 1: HERO CARD (Gồm Tên & Status Badge) */}
        <div className="md:col-span-2 md:row-span-2 bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div className="z-10">
            {/* --- Status Badge với spam click handle --- */}
            <motion.div 
            ...
            {/* Nếu BSOD, hiện BSOD overlay thu nhỏ lên thẻ (Meme tối ưu) */}
            {bsodState && (
            ...
            <h1 className="text-5xl md:text-6xl font-black tracking-tight text-white mb-2 flex items-center gap-3">
              <span 
                ...
                onMouseLeave={stopHackerHold} // Đề phòng di chuột ra ngoài khi đang giữ
              >
                An Phan
              </span>
...
<       Link href="/blog" className="md:col-span-1">
        ...
        {/* BENTO 5: RICKROLL COOKIE (Đã sửa) */}
        <motion.a 
      ...
      <footer className="w-full max-w-5xl py-8 border-t border-white/5 text-xs text-slate-600 flex justify-between items-center transition-colors hacker:text-green-800">
        <div className="flex gap-4">
          <span>wakeupTeddy...</span>
          <span>© 2026 An Phan</span>
        </div>
        
        {/* Guitar đứng yên, chỉ nảy nhẹ và đổi màu khi hover */}
        <motion.span 
